...
public class AdvancedMIDIMessageIO  extends PApplet{
	...
	MidiBus speedBus, newMoveBus, resultBus; // The MidiBus

	public void setup() {
	  size(400, 400);
	  background(0);
	  speedBus = new MidiBus(this,0,1);
			  //this, "SpeedBus");
	  newMoveBus = new MidiBus(this,1,2);
	  resultBus = new MidiBus(this, 2,3);
	...
	public void draw() {
	  ...
	  while(true){
		    ...
		    delay(4000); //Now let's wait 2 sec before we start again
		 // speedBus.sendNoteOn(channel, pitch, velocity);
		    speedBus.sendNoteOn(channel, ivalue, velocity);
		   // speedBus.sendControllerChange(new ControlChange(0, 5, ivalue ));
		    System.out.println(ivalue);
		    ivalue+=1;
		    if(ivalue<0) ivalue=0;
		  delay(500);
		  speedBus.sendNoteOff(channel, ivalue, velocity);
